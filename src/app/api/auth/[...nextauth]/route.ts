...
export const authOptions: NextAuthOptions = {
  providers: [
    CredentialsProvider({
      ...
      async authorize(credentials): Promise<ExtendedUser | null> {
        ...
        try {
          const response = await fetch(`${envConfig.BACKEND_API_URL}/auth/login`, {
            method: "POST",
            headers: {
  ...
  callbacks: {
    ...
    //     try {
    //       const response = await fetch(`${envConfig.BACKEND_API_URL}/auth/refresh`, {
    //         method: "POST",
    //         headers: {
    //           "Content-Type": "application/json",
    //         },
    ...
    async jwt({ token, user, account, trigger, session }) {
      if (user) {
        const extendedUser = user as ExtendedUser;
        // Sync initial user data from backend (no token generation here)
        token["id"] = extendedUser.id;
        token.email = extendedUser.email;
        token["firstName"] = extendedUser.firstName;
        token["lastName"] = extendedUser.lastName;
        ...
        token["isEmailVerified"] = extendedUser.isEmailVerified;
        token["profilePicture"] = extendedUser.profilePicture;
        // Store backend tokens but don't manage them
        token["accessToken"] = extendedUser.accessToken;
        token["refreshToken"] = extendedUser.refreshToken;
      }

      // For Google OAuth, sync with backend as before
      if (account?.provider === "google") {
        try {
          const response = await fetch(`${envConfig.BACKEND_API_URL}/auth/google`, {
            method: "POST",
      ...
      // For session updates (e.g., role changes)
      if (trigger === "update" && session) {
        return { ...token, ...session };
      }

      // No token refresh here—let your backend handle it via API calls in your app/middleware
      return token;
    },
  ...
  events: {
    ...
    async signOut({ token }) {
      ...
      if (token?.["refreshToken"]) {
        try {
          await fetch(`${envConfig.BACKEND_API_URL}/auth/logout`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
