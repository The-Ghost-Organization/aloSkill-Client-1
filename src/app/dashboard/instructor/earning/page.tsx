        ...
        <div className="min-h-screen flex flex-col gap-4">
            ...
            <div className="grid grid-cols-4 gap-4">
                {stats.map((stat, index) => (
                    <div key={index} className="bg-white rounded p-2 flex items-center gap-3">
                ...
                <div className='w-full h-[380px] flex gap-4 items-center'>
                    ...
                    <div className="bg-white rounded w-[40%] h-full overflow-y-auto">
                        ...
                        <div className="p-4 w-full flex flex-col gap-2">
                            {cards[selectedCard] && (
                                <div className={`bg-gradient-to-br ${cards[selectedCard].color} rounded p-4 text-white`}>
                                    {/* Card Brand */}
                                ...
                                <div className='flex items-center gap-2'>
                                    {cards.map((_, index) => (
                                        <button
                                            ...
                                            className={`w-2 h-2 rounded-full transition-all ${selectedCard === index ? 'bg-orange-500 w-8' : 'bg-gray-300'
                                                }`}
                                        />
                                    ))}
                                </div>
                        ...
                        <div>
                            ...
                            <div className="flex flex-col gap-2 p-4">
                                ...
                                {paymentMethods.map((method, index) => (
                                    <div key={index} className="flex items-center justify-between px-2 py-1 border border-gray-200 rounded hover:border-gray-300 transition-colors">
                                        <div className="flex items-center space-x-4">
                                            {/* Logo */}
                                            <div className={`w-12 h-8 ${method.logo === 'VISA' ? 'bg-blue-600' : 'bg-red-500'} rounded flex items-center justify-center text-white font-bold text-xs`}>
                                                {method.logo}
                                            </div>
                                ...
                                {withdrawalHistory.map((item, index) => (
                                    <div key={index} className="p-2 hover:bg-gray-50 transition-colors">
                                        <div className="grid grid-cols-12 items-center">
                                            ...
                                            <div className="col-span-3 flex gap-4 items-start text-xs">{item.company}</div>
                                            <div className={`col-span-2 flex gap-4 items-start text-xs p-1 rounded ${item.statusColor}`}>{item.status}</div>
                                        </div>
                                    </div>
                                ))}
                            </div>
